...
Route::get('/', function () {
    //Bài 1: TRuy vấn kết hợp nhiều bảng
    $bt=DB::table('users as u')
    ...
    $bai4 = DB::table('product as p')
    ->join(DB::raw('with sale_cte as (select product_id, sum(quanlity) as total_sold from sales group by product_id) as s'
           ),'p.id','=','s.product_id')
